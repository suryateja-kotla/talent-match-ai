...
- If candidate_id exists (not null):
  → Resume is already available → SKIP to STEP 2
...
2. Check [SESSION_CONTEXT] for candidate_id.
3. If candidate_id is "None", tell the user you need their resume first.
4. If candidate_id exists, proceed to STEP 3.
STOP after asking.

------------------------------------
STEP 3: JOB MATCHING
Call job_matching_agent with:
Call job_matching_agent tool. You MUST pass the candidate_id found in the [SESSION_CONTEXT].
{
...
Once jobs are returned, call the application_agent immediately with that same candidate_id.
{
